...
public class Manager : MonoBehaviour
{
    ...
    int maxPotions = 2;
    int currentPotion = 0;
    public void SetSelectedPotion(Pocion pocion)
    {
        if (currentPotion == maxPotions - 1)
        {
            currentPotion = 0;
        }
        Pocion aux = selectedColorPotions[currentPotion];
        if(aux != null)
            aux.ResetPosition();
            
        selectedColorPotions[currentPotion] = pocion;
